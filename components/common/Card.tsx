...
import { Card as HeroCard, ProgressBar, Skeleton } from "@heroui/react";
...
export const CardSkeleton = () => {
    return (
        <HeroCard
            className="relative flex flex-col rounded-[22px] overflow-hidden bg-[#0A0B0F] border border-[#24262E] w-[300px] h-[430px] p-0 shadow-none"
        >
            <div className="p-0 overflow-hidden relative h-[400px]">
                <Skeleton className="h-full w-full bg-[#1A1C23]" />
            </div>
            <HeroCard.Footer className="flex flex-col px-6 pb-3 pt-0 bg-[#0A0B0F] relative z-20 flex-1 justify-end items-stretch gap-2 border-none">
                <div className="mb-auto pt-1 flex flex-col gap-2">
                    <Skeleton className="h-5 w-3/4 rounded-lg bg-[#1A1C23]" />
                    <Skeleton className="h-4 w-1/2 rounded-lg bg-[#1A1C23]" />
                    <div className="flex gap-2 mt-1">
                        <Skeleton className="h-3 w-10 rounded-lg bg-[#1A1C23]" />
                        <Skeleton className="h-3 w-10 rounded-lg bg-[#1A1C23]" />
                        <Skeleton className="h-3 w-10 rounded-lg bg-[#1A1C23]" />
                    </div>
                </div>
                <div className="flex justify-between items-center mb-2">
                    <Skeleton className="h-7 w-24 rounded-full bg-[#1A1C23]" />
                    <Skeleton className="h-5 w-10 rounded-lg bg-[#1A1C23]" />
                </div>
                <div className="flex flex-col gap-2">
                    <div className="flex justify-between">
                        <Skeleton className="h-3 w-16 rounded-lg bg-[#1A1C23]" />
                        <Skeleton className="h-4 w-10 rounded-lg bg-[#1A1C23]" />
                    </div>
                    <Skeleton className="h-2 w-full rounded-full bg-[#1A1C23]" />
                </div>
            </HeroCard.Footer>
        </HeroCard>
    );
...
const Card: React.FC<PlayerProps> = ({ player, rank }) => {
    const rankStr = `#${rank}`;
    const styles = getRankStyles(rankStr);
    const categoryStyles = getCategoryStyles(player.category?.name || "Uncategorized");
            ...
            >
                    ...
                    <div className="mb-auto pt-1 flex flex-col gap-1">
                        ...
                        <div className="flex items-center gap-2.5 mb-2">
                            <div className="flex items-center gap-1.5 text-[#7B899D] text-[10px] outfit">
                                ...
                                <span className="whitespace-nowrap">{player.appearancesGames}</span>
                            </div>
                            <div className="flex items-center gap-1.5 text-[#7B899D] text-[10px] outfit">
                                <FiCalendar className="text-[#00CCFF] text-sm" />
                                <span className="whitespace-nowrap">{player.era}</span>
                            </div>
                            {player.country && (
                                <div className="flex items-center gap-1.5 text-[#7B899D] text-[10px] outfit">
                ...
                text-[11px] font-medium tracking-widest uppercase ${categoryStyles}`}
                        ...
                        <div className="flex items-center gap-1 text-[#7B899D]">
                            ...
                            <span className="font-medium text-[#7B899D] text-[12px]">{player.championships}</span>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-1.5 min-w-[160px] md:min-w-[130px] w-full md:w-auto">
                        ...
                        >
                            <div className="flex items-center justify-between w-full">
                                ...
                                <span className="orbitron text-[14px] font-bold text-[#00CCFF]">{(player.finalScore || 0).toFixed(1)}</span>
                            </div>
