...
const Card: React.FC<PlayerProps> = ({ player }) => {
    ...
    const getCategoryIcon = () => {
        switch (player.category) {
            case "MUSIC": return <span className="text-lg">🎤</span>;
            case "RUGBY": return <span className="text-lg">🏉</span>;
            default: return <IoMdFootball className="text-[#00CCFF] text-xl" />;
        }
    };
    ...
    return (
        ...
        >
            ...
            <div className="absolute top-0 left-0 right-0 flex justify-between items-start z-30  px-3 py-2 pointer-events-none">
              ...
              font-[700] text-[14px] tracking-tighter
              ${styles.rankColor} shadow-2xl`}
                ...
                <div
                    className="bg-[#00CCFFE5] text-[#0B0B0F] font-[700] px-3 py-1.5 rounded-[12px] 
            ...
            <HeroCard.Footer className="flex flex-col px-6 pb-3 pt-0 bg-[#0A0B0F] relative z-20 flex-1 justify-end items-stretch gap-2 border-none">
                <div className="mb-auto pt-1 flex flex-col gap-1">
                    <h3 className="orbitron text-[16px] font-[700] text-[#E7EBEF] tracking-wider mb-0.5 leading-tight">
                        {player.name}
                    </h3>
                    <p className="text-[#7B899D] text-[13px] mb-2 font-[400]  outfit">
                        {player.role}
                    </p>
              ...
              text-[11px] font-[500] tracking-widest uppercase ${categoryStyles}`}
                    >
                        {player.category}
                    </div>
                    ...
                    <div className="flex items-center gap-1 text-[#7B899D]">
                        ...
                        <span className="font-[500] text-[#7B899D] text-[12px]">{player.trophies}</span>
                    </div>
                </div>
                   ...
                   <div className="flex flex-col gap-1.5 min-w-[160px] md:min-w-[130px] w-full md:w-auto">
                                                            ...
                                                            >
                                                                <div className="flex items-center justify-between w-full">
                                                                    <span className="outfit font-[400] text-[12px] tracking-wider text-[#7B899D]">HGW Score</span>
                                                                    <span className="orbitron text-[14px] font-[700] text-[#00CCFF]">{player.score.toFixed(1)}</span>
                                                                </div>
