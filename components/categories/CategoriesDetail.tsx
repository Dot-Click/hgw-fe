...
import { Card, cn, Button, Tooltip, Spinner, toast } from '@heroui/react'
...
const CategoriesDetail = () => {
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("Are you sure? This will also delete all players in this category!")) {
      try {
        await dispatch(deleteCategory(id)).unwrap();
        toast.success("Category and associated players deleted.");
      } catch (err: any) {
        toast.danger(err || "Failed to delete category");
      }
    }
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <Card className="bg-[#111A2C]/40 border border-[#1E293B] p-6 rounded-2xl flex flex-row items-center gap-5 shadow-none group transition-all">
          ...
          <div className="flex flex-col gap-0.5">
            ...
            <span className="text-[24px] font-black text-white orbitron tracking-tight">
              {categories.reduce((acc, cat) => acc + (cat._count?.players || 0), 0)}
            </span>
          </div>
        </Card>
      </div>
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[12px] text-zinc-500 outfit">
                    {category._count?.players || 0} {category._count?.players === 1 ? 'player' : 'players'}
                  </span>
                </div>
              </div>
                ...
                <div className="flex items-center gap-1">
                  <Tooltip>
                    <Button
                      ...
                      className="text-zinc-500 hover:text-white transition-colors h-9 w-9 min-w-0 bg-transparent border-none"
                    >
                      <FiEdit3 size={18} />
                    </Button>
                    <Tooltip.Content placement="top" className="bg-[#1A2333] text-white orbitron text-[10px] rounded-lg border border-[#1E293B] px-3 py-1.5 shadow-xl">
                      Edit Category
                    </Tooltip.Content>
                  </Tooltip>
                  ...
                  <Tooltip>
                    <Button
                      ...
                      className="text-zinc-500 hover:text-red-500 transition-colors h-9 w-9 min-w-0 bg-transparent border-none"
                    >
                      <FiTrash2 size={18} />
                    </Button>
                    <Tooltip.Content placement="top" className="bg-red-950 text-red-200 orbitron text-[10px] rounded-lg border border-red-900/50 px-3 py-1.5 shadow-xl">
                      Delete Category
                    </Tooltip.Content>
                  </Tooltip>
                </div>
